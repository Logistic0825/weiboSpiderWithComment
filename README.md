...
- 启用条件：请在 config.json 的 write_mode 中包含 "json"
  - 示例：`"write_mode": ["csv", "txt", "json"]`
- 保存位置：评论写入到与微博同一个 JSON 文件
  - 路径规则：`{输出目录或当前目录}/weibo/{昵称或用户id}/{用户id}.json`
  - 当 `result_dir_name=0` 使用昵称目录；当 `result_dir_name=1` 使用用户id目录
- 数据结构：在 JSON 中新增 `comments` 字段，按原微博 `weibo_id` 分组
  - 示例：
    ```json
    {
      "user": { ... },
    ...
    ```
- 抓取范围：默认抓取每条微博前 3 页评论。如需更大范围，可自行调整代码中的 `max_pages` 参数。
- 访问限制：如 cookie 失效或被限速，可能暂时无法抓到评论；请适当降低抓取频率或更新有效 cookie。
